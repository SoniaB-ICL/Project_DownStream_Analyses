...
#Total reads- should not deviate sig from norm- done-done
#Mean aligned read length- avoid large deviations


#After subsetting data to 1 sample per participant, outlier detection causing skewness was checked for

# Filter extreme upper outliers for the 'pct_adapter' metric
extreme_upper_pct_adapter <- melt_RS %>%
  filter(variable == "PCT_PF_READS_ALIGNED") %>%     
  mutate(
...
print(extreme_upper_pct_adapter)


# Filter extreme lower outliers 
extreme_lower_pct_adapter <- melt_RS %>%
  filter(variable == "PF_READS") %>%      
  mutate(
    Q1 = quantile(value, 0.25),
    IQR = IQR(value),
    lower_bound = Q1 - 3 * IQR               
  ) %>%
  filter(value < lower_bound) %>%           
  select(SampleID, value)                   

print(extreme_lower_pct_adapter)
